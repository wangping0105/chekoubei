  ...
  def create_attachments_by(entity, attachments = {})
    attachments.each do |key, val|
      if params[key].present?
        ...
        _operation_log_service = (OperationLogService::Logging.by_loggable_klass(entity.class.name, operator: current_user) rescue nil)
        _operation_log_service.try(:logging_attachment_create, entity, attachments) if attachments.present? && _operation_log_service.present?
      end
    end
  end
    ...
    attachments.each do |key|
      Attachment.where(id: params[key]).each do |attachment|
        if attachment.destroy
          OperationLogService::Logging.by_loggable_klass(attachment.attachmentable.class.name, operator: current_user).try(:logging_attachment_destroy, attachment.attachmentable, attachment) if attachment.attachmentable.present?
        end
      end
    end
  end
